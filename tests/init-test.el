...
(defvar my/terminal-osc52-remote-only)
...
(declare-function my/terminal-finish-editor-buffer "config-terminal")
...
(declare-function my/terminal-ssh-session-p "config-terminal")
...
(declare-function my/mcp-maybe-start-emacs-server "config-mcp")
...
(ert-deftest emacs-config/make-user-target-documents-user-setup ()
  (with-temp-buffer
    ...
    (let ((makefile (buffer-string)))
      ...
      (should (string-match-p "^USER_EDITOR_COMMAND \\?=" makefile))
  ...
  (let ((user-helper (expand-file-name "scripts/user.el"
    ...
    (with-temp-buffer
      ...
      (let ((script (buffer-string)))
        ...
        (should (string-match-p "USER_EDITOR_COMMAND" script))
        (should (string-match-p "USER_SHELL_FILE" script))
        (should (string-match-p "USER_TMUX_FILE" script))
        (should (string-match-p "unmanaged by this helper" script))
...
(ert-deftest emacs-config/terminal-ssh-session-detects-remote-editor-sessions ()
  (let ((process-environment '("SSH_CONNECTION=127.0.0.1 1 127.0.0.1 2")))
    (should (my/terminal-ssh-session-p)))
  (let ((process-environment nil))
    (should-not (my/terminal-ssh-session-p))))
...
(ert-deftest emacs-config/terminal-osc52-keeps-local-platform-clipboard-pair ()
  (should-not (my/terminal-frame-p))
  (let ((interprogram-cut-function 'platform-copy)
        (interprogram-paste-function 'platform-paste)
        (my/terminal-osc52-remote-only t)
    ...
    (cl-letf (((symbol-function 'display-graphic-p)
      ...
      (should (eq interprogram-cut-function 'platform-copy))
      (should (eq interprogram-paste-function 'platform-paste)))))

(ert-deftest emacs-config/terminal-osc52-can-copy-when-no-local-paste-exists ()
  (let ((interprogram-cut-function nil)
    ...
    (cl-letf (((symbol-function 'display-graphic-p)
      ...
      (should-not interprogram-paste-function))))

(ert-deftest emacs-config/terminal-osc52-disables-external-paste-remotely ()
  (let ((interprogram-cut-function 'platform-copy)
        (interprogram-paste-function 'platform-paste)
        (my/terminal-osc52-remote-only t)
        (process-environment '("SSH_TTY=/dev/ttys001")))
    (cl-letf (((symbol-function 'display-graphic-p)
               (lambda (&optional frame)
                 (ignore frame)
                 nil)))
...
(ert-deftest emacs-config/terminal-matched-clipboard-pair-preserves-kill-yank ()
  (let* ((external-clipboard "stale pasteboard")
         (kill-ring nil)
         (kill-ring-yank-pointer nil)
         (interprogram-cut-function
          (lambda (text)
            (setq external-clipboard text)
            nil))
    ...
    (with-temp-buffer
      ...
      (kill-line)
      (yank)
      (should (equal (buffer-string) "fresh kill")))))

(ert-deftest emacs-config/terminal-osc52-without-paste-preserves-kill-yank ()
  (let ((kill-ring nil)
        (kill-ring-yank-pointer nil)
        (interprogram-cut-function (lambda (_text) nil))
        (interprogram-paste-function nil))
    (with-temp-buffer
      (insert "fresh kill")
      (goto-char (point-min))
...
(ert-deftest emacs-config/terminal-editor-environment-is-unmanaged-by-default ()
  (should-not my/terminal-editor-command)
  (should-not (my/terminal-editor-environment)))

(ert-deftest emacs-config/terminal-editor-environment-is-applied ()
  (let ((process-environment nil)
        (my/terminal-editor-command "emacs -nw"))
    (my/terminal-apply-editor-environment)
    (should (equal (getenv "EDITOR") "emacs -nw"))
    (should (equal (getenv "VISUAL") "emacs -nw"))
    (should (equal (getenv "GIT_EDITOR") "emacs -nw"))))

(ert-deftest emacs-config/terminal-editor-environment-leaves-existing-values-alone ()
  (let ((process-environment '("EDITOR=vim"))
        (my/terminal-editor-command nil))
    (my/terminal-apply-editor-environment)
    (should (equal (getenv "EDITOR") "vim"))
    (should-not (getenv "VISUAL"))
    (should-not (getenv "GIT_EDITOR"))))

(ert-deftest emacs-config/terminal-server-start-is-opt-in ()
  (should-not my/terminal-start-server)
  (let ((my/terminal-start-server nil)
        (noninteractive nil)
        (started nil))
...
(ert-deftest emacs-config/terminal-quick-editor-finish-dispatches-with-editor ()
  (let ((with-editor-mode t)
        (called nil))
    (cl-letf (((symbol-function 'with-editor-finish)
               (lambda (&optional arg)
                 (setq called arg))))
      (my/terminal-finish-editor-buffer 'force)
      (should (eq called 'force)))))

(ert-deftest emacs-config/terminal-quick-editor-finish-dispatches-server-edit ()
  (let ((with-editor-mode nil)
        (server-buffer-clients '(fake-client))
        (called nil))
    (cl-letf (((symbol-function 'server-edit)
               (lambda (&optional arg)
                 (setq called arg))))
      (my/terminal-finish-editor-buffer 'done)
      (should (eq called 'done)))))

(ert-deftest emacs-config/terminal-quick-editor-finish-preserves-normal-exit ()
  (let ((with-editor-mode nil)
    ...
    (cl-letf (((symbol-function 'save-buffers-kill-terminal)
               (lambda (&optional arg)
                 (setq called arg))))
      (my/terminal-finish-editor-buffer 'exit)
      (should (eq called 'exit)))))
...
(ert-deftest emacs-config/terminal-git-commit-finish-bindings-are-present ()
  ...
  (should (eq (lookup-key git-commit-mode-map (kbd "C-c C-k"))
              'with-editor-cancel))
  (should (eq (lookup-key git-commit-mode-map (kbd "C-x C-c"))
              'my/terminal-finish-editor-buffer))
  (should (eq (lookup-key with-editor-mode-map (kbd "C-x C-c"))
              'my/terminal-finish-editor-buffer)))
...
(ert-deftest emacs-config/terminal-environment-docs-cover-terminal-setup ()
  (with-temp-buffer
    ...
    (let ((readme (buffer-string)))
      ...
      (should (string-match-p "C-x #" readme))
      (should (string-match-p "C-x C-c" readme))
...
(ert-deftest emacs-config/mcp-starts-emacs-server-on-demand ()
  (let ((started nil))
    (cl-letf (((symbol-function 'server-running-p)
               (lambda (&optional name)
                 (ignore name)
